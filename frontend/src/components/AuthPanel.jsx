...
// Centered auth panel modal. Tab-switches between Login and Register with a sliding animation.
export default function AuthPanel({ open, onClose, defaultTab = "login", onAuthSuccess }) {
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm drawer-overlay"
        onClick={onClose}
        aria-hidden="true"
      />
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 pointer-events-none">
        {/* Panel */}
        <div
          className="w-full max-w-md flex flex-col bg-white dark:bg-gray-950 shadow-2xl rounded-2xl animate-scale-in pointer-events-auto overflow-hidden max-h-full"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          {/* Scrollable body */}
          <div className="flex-1 overflow-y-auto overflow-x-hidden p-6 pb-2">
            {/* Tab switcher */}
            <div className="flex rounded-xl border border-gray-200 dark:border-gray-700 p-1 gap-1 mb-6">
            ...
            {serverError && (
              <div className="rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 px-4 py-3 text-sm text-red-700 dark:text-red-400 mb-4">
                {serverError}
              </div>
            )}
