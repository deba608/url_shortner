  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white">My URLs</h1>
          <p className="mt-1 text-sm text-gray-400">Manage, share, and track your links.</p>
        </div>
        {!loading && !error && (
      ...
      {!loading && error && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-8 text-center">
          ...
          <button
            onClick={refetch}
            className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-indigo-400 hover:underline"
          >
            Try again →
          </button>
        </div>
      ...
      {!loading && !error && urls.length === 0 && (
        <div className="rounded-xl border border-dashed border-white/10 p-16 text-center">
          <div className="mx-auto mb-4 h-16 w-16 rounded-xl bg-indigo-500/10 flex items-center justify-center">
